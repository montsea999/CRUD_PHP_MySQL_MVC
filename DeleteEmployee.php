...
//creo la clase DeleteEmployee que hereda de la clase Connection y cuando el flujo llegue aquí, ejecutará implícitamente el constructor de Connector, conectando con la bbdd
class DeleteEmployee extends Connection{   
    // FUNCIONALIDAD DELETE del CRUD
    function delete($dniToDelete){
        
        //query de tipo DELETE
        $sql = "DELETE FROM proyecto_daw.employee WHERE employeeDni = ?";
        //preparo la query
        ...
        if($deleted>0){
            echo "<br><br><br><br><h1 class='flowerRed'>Se ha(n) eliminado " . $deleted . " empleado(s)";
        } else {
            echo "<br><br><br><br><h1 class='flowerRed'>No se ha eliminado ningún empleado";
        }
    }
}
    //creo la variable que recupera a través de la url el Dni del employee que quiero eliminar
    $employeeDni=$_GET['employeeDni']; 

if (isset($employeeDni)) {
    $deleteEmployee = new DeleteEmployee(); //instancio la clase DeleteEmployee
    
    //y ahora puedo utilizar la función delete pasándole por parámetro el Dni recuperado a través de la url
    $deleteEmployee->delete($employeeDni);
}
